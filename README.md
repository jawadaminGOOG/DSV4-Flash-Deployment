...
* **The attention block table must fit in SMEM, which is 1 MiB per core.** It is prefetched at 4 bytes per 16-token block, and it shares that 1 MiB with about 0.11 KiB of scheduler arrays per sequence. At `--max-num-seqs 512` and `--max-model-len 9216` the engine asks for `512 × (9216 / 16) × 4 = 1,179,648` bytes for the table alone and fails to start with `RESOURCE_EXHAUSTED ... space=smem`. Three values measured at 9,216 context: 256 uses 603.1K, 384 uses 906.3K, and 432 needs 1028.6K and fails by 4.6K. The practical ceiling there is near `--max-num-seqs 416`. Raise the context and you must lower the sequence ceiling to match. This limit is independent of HBM and it is the one that binds as context grows.
* **`--max-num-seqs 512` is a ceiling, not a preference.** That buffer scales linearly with the flag, so `--max-num-seqs 1024` asks for 7.89 GiB and the engine fails to start with `RuntimeBufferAllocationFailure`. Separately, the server prints `GPU KV cache size` **per DP rank**, not per slice. At 2,048 context it reports `81,427 tokens` per rank, so the slice holds 1,302,832 tokens, or **636 concurrent sequences at the full 2,048-token context** — and values much above ~640 could not be filled at this context length in any case. At 9,216 context it reports 180,744 per rank, so the slice holds 2,891,904 tokens, or **313 concurrent sequences at full length**.
