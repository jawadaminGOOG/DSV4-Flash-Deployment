...
Measures three workload shapes — balanced, prefill-heavy and reasoning — at ten
concurrency levels from C=1 to C=512, and writes one JSON row per point.

Run it inside the serving pod, against `localhost:8000`, so the client adds no
network of its own.
...
Usage: benchmark_sweep.py <shape> <out.json> [concurrencies...]
...
# 9216 is exactly 1024 + 8192, so both 8k shapes fit it exactly.
MAX_MODEL_LEN = 9216

DEFAULT_CONCURRENCIES = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]

# Request count per point, and the ISL/OSL. The reasoning shape gets one
# request at C=1, because one request already emits 8192 tokens, which takes
# about four minutes.
SHAPES = {
...
def main():
    if len(sys.argv) < 3 or sys.argv[1] not in SHAPES:
        sys.exit(f"usage: benchmark_sweep.py <{'|'.join(SHAPES)}> <out.json> [C...]")
